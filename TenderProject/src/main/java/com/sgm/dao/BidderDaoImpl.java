...
public class BidderDaoImpl implements BidderDao{
	...
	public String acceptBid(String applicationId,String tenderId,String vendorId) {
		...
		try {
			String sql="select vemail from vendor where vid=?";
			PreparedStatement ps1= con.prepareStatement(sql);
			ps1.setString(1,vendorId );
			ResultSet rs1=ps1.executeQuery();
			while(rs1.next()) {
				mail=rs1.getString("vemail");
				System.out.println(mail);
			}
			EmailNotificationUtil.sendBidAcceptanceEmail(mail,tid);
		}catch(Exception e) {
			e.printStackTrace();
		}
		finally{
